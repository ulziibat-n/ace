...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'flex flex-col h-full relative group/entry bg-white rounded-sm overflow-hidden transition-all duration-300 ' . $ub_card_class ); ?>>
	...
	<div class="flex grow flex-col p-6">
		...
		<h3 class="mb-4 text-sm leading-tight font-bold transition-colors group-hover/entry:text-primary">
			<a href="<?php the_permalink(); ?>" class="block no-underline before:absolute before:inset-0 before:z-10">
				<?php the_title(); ?>
			</a>
		</h3>
		<div class="mt-auto flex items-center text-[0.65rem] font-bold text-slate-500 uppercase transition-all group-hover/entry:text-primary">
			<span class="font-medium text-slate-500"><?php echo esc_html( get_the_date() ); ?></span>
			<span class="mx-2 font-light text-slate-500 opacity-50">|</span>
			<div class="flex items-center gap-2 transition-all">
				<span><?php esc_html_e( 'Нийтлэл унших', 'aceedu' ); ?></span>
				<svg class="h-3 w-3 transition-transform group-hover/entry:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"></path></svg>
			</div>
		</div>
	</div>
</article>
