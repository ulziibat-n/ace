...
<article id="school-<?php echo esc_attr( $ub_post_id ); ?>" <?php post_class( 'flex flex-col h-full group/entry bg-white rounded-sm overflow-hidden transition-all duration-300 border border-primary/5 hover:shadow-xl ' . $ub_card_class ); ?>>
	...
	<header class="relative aspect-4/3 overflow-hidden bg-slate-100">
		...
		<?php if ( $ub_location ) : ?>
			<div class="absolute top-4 left-4">
				<span class="rounded-xs bg-white px-2 py-1.5 text-[0.625rem] leading-none font-bold text-primary uppercase shadow-sm">
	...
	<div class="flex grow flex-col p-6">
		...
		<div class="mb-4 flex flex-wrap gap-2">
			<?php if ( $ub_name_ko ) : ?>
				<span class="rounded-xs bg-slate-50 px-2 py-0.5 text-[0.7rem] font-medium text-foreground/40">
					<?php echo esc_html( $ub_name_ko ); ?>
				</span>
			<?php endif; ?>
		</div>

		<h3 class="mb-2 line-clamp-2 text-base leading-tight font-bold transition-colors group-hover/entry:text-primary">
