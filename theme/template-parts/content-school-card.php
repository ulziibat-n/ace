...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'school-card relative flex flex-col h-full group/entry bg-white rounded-xs overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1' ); ?>>
	...
	<!-- Image / Logo Area -->
	<header class="overflow-hidden relative aspect-video bg-slate-50">
		<?php if ( $ub_is_featured ) : ?>
			<div class="absolute top-4 left-4 z-20">
				<span class="bg-amber-400 text-amber-950 text-[0.625rem] font-bold uppercase leading-none px-2 py-1.5 rounded-xs shadow-sm">
		...
		<!-- Logo Overlay -->
		<?php if ( $ub_logo ) : ?>
			<div class="absolute bottom-4 left-4 w-12 h-12 p-1 bg-white rounded-xs shadow-lg flex items-center justify-center overflow-hidden z-20 border border-slate-100">
				<?php echo wp_get_attachment_image( $ub_logo, 'thumbnail', false, array( 'class' => 'max-w-full max-h-full object-contain' ) ); ?>
			</div>
		<?php endif; ?>


		<div class="absolute inset-0 bg-linear-to-t from-slate-900/40 to-transparent pointer-events-none"></div>
	</header>
	...
	<div class="flex flex-col grow p-6">
		...
		<div class="flex flex-wrap gap-2 mb-4">
			<?php if ( $ub_location ) : ?>
				<div class="flex items-center text-[0.625rem] font-bold uppercase tracking-wider text-slate-400">
					<svg class="w-3 h-3 mr-1 text-primary/50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
					<?php echo esc_html( $ub_location[0]->name ); ?>
				</div>
			<?php endif; ?>

			<?php if ( $ub_guarantor ) : ?>
				<div class="flex items-center text-[0.625rem] font-bold uppercase tracking-wider text-emerald-600/80">
		...
		<h3 class="mb-3 text-base font-bold leading-tight transition-colors group-hover/entry:text-primary">
		...
		<?php if ( $ub_short_intro ) : ?>
			<p class="text-slate-500 text-[13px] line-clamp-2 mb-6 leading-relaxed">
				<?php echo esc_html( $ub_short_intro ); ?>
			</p>
		<?php endif; ?>

		<!-- Footer / Read More -->
		<div class="flex justify-between items-center pt-4 mt-auto text-[0.65rem] font-bold border-t transition-all border-foreground/5 text-primary group-hover/entry:gap-2">
			<span class="uppercase tracking-widest"><?php esc_html_e( 'Дэлгэрэнгүй', 'ace' ); ?></span>
