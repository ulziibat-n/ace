...
$ub_title_mw       = get_field( 'cta_title_max_width' );
$ub_description_mw = get_field( 'cta_description_max_width' );

if ( ! $ub_title ) {
	$ub_title = 'Таны Солонгос дахь ирээдүй эндээс эхэлнэ';
}
if ( ! $ub_description ) {
	$ub_description = 'Өнөөдөр анхны алхмаа хийж, мэргэжлийн зөвлөхтэйгөө холбогдоорой.';
}
...
if ( ! $ub_description_mw ) {
	$ub_description_mw = 720;
}

if ( ! $ub_title && $is_preview ) {
	echo '<div class="p-4 text-center bg-slate-100 border-2 border-dashed border-slate-300 rounded-sm mb-6 text-neutral-900">СTA Баннер: Мэдээллээ оруулна уу.</div>';
}
?>
...
<section id="<?php echo esc_attr( $ub_id ); ?>" class="<?php echo esc_attr( $ub_class_name ); ?> relative overflow-hidden bg-primary text-white py-16 lg:py-24">
	...
	<div class="container relative z-10">
		<div class="max-w-4xl mx-auto text-center flex flex-col items-center">
			<?php if ( $ub_title ) : ?>
				<h2 class="text-3xl lg:text-4xl font-bold leading-tight mb-3 text-white mx-auto" 
			...
			<?php if ( $ub_description ) : ?>
				<p class="text-lg md:text-xl mb-6 text-white mx-auto" 
			...
			<div class="flex flex-col sm:flex-row gap-3 mt-4 items-center justify-center">
				<?php if ( $ub_link_1 ) : ?>
					<a href="<?php echo esc_url( $ub_link_1['url'] ); ?>" 
						...
						class="inline-flex items-center justify-center bg-white text-primary px-6 py-2 rounded-xs font-bold transition-all duration-300 hover:bg-neutral-100 shadow-xl shadow-black/5 no-underline border border-white">
						<!-- Speech Bubble Icon. -->
				...
				<?php else : ?>
					<!-- Default button if none provided in preview. -->
					<div class="inline-flex items-center justify-center bg-white text-primary px-6 py-2 rounded-xs font-bold opacity-80 cursor-default border border-white">
						<svg class="w-4 h-4 mr-3 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path>
						</svg>
				...
				<?php else : ?>
					<!-- Default button if none provided in preview. -->
					<div class="inline-flex items-center justify-center bg-transparent border-2 border-white/30 text-white px-6 py-2 rounded-xs font-bold opacity-80 cursor-default min-w-[140px]">
						<svg class="w-5 h-5 mr-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path>
						</svg>
