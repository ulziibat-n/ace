<?php
/**
 * Schools Carousel Block Template.
 *
 * @package ace
 * @since 1.0.0
 */
...
$ub_id = 'schools-carousel-' . $block['id'];
if ( ! empty( $block['anchor'] ) ) {
	$ub_id = $block['anchor'];
}

$ub_align      = ! empty( $block['align'] ) ? 'align' . $block['align'] : 'alignfull';
$ub_class_name = 'block-schools-carousel relative w-full overflow-hidden bg-white py-16 lg:py-24 ' . $ub_align . ( isset( $block['className'] ) ? ' ' . $block['className'] : '' );

// ACF Fields.
$ub_example_data = ub_get_block_example_data( $block );
$ub_headline     = get_field( 'sc_title' ) ?: ( $ub_example_data['sc_title'] ?? '' );
$ub_description  = get_field( 'sc_description' ) ?: ( $ub_example_data['sc_description'] ?? '' );
$ub_posts_count  = get_field( 'sc_posts_count' ) ?: 10;
$ub_filter_type  = get_field( 'sc_filter_type' ) ?: 'all';
$ub_link         = get_field( 'sc_link' );

// Max Width Logic.
$ub_title_mw = get_field( 'sc_title_max_width' ) ?: 768;
$ub_desc_mw  = get_field( 'sc_description_max_width' ) ?: 720;
...
if ( 'manual' === $ub_filter_type ) {
	$ub_selected = get_field( 'sc_selected_schools' );
	if ( ! empty( $ub_selected ) ) {
		$ub_args['post__in'] = $ub_selected;
		$ub_args['orderby']  = 'post__in';
	}
} elseif ( 'taxonomy' === $ub_filter_type ) {
	$ub_locations = get_field( 'sc_location_filter' );
	$ub_types     = get_field( 'sc_type_filter' );
	$ub_tax_query = array( 'relation' => 'AND' );

	if ( ! empty( $ub_locations ) ) {
		$ub_tax_query[] = array(
	...
	if ( count( $ub_tax_query ) > 1 ) {
		$ub_args['tax_query'] = $ub_tax_query;
	}
}
