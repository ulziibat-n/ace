...
if ( empty( $ub_testimonials ) && $is_preview ) {
	echo '<div class="p-12 text-center rounded-sm border-2 border-dashed bg-slate-100 border-slate-300">Testimonials: Мэдээллээ оруулна уу.</div>';
	return;
}
...
<section id="<?php echo esc_attr( $ub_id ); ?>" class="<?php echo esc_attr( $ub_class_name ); ?> relative w-full overflow-hidden py-16 lg:py-24">
	<div class="container">
		<div class="flex flex-row gap-8 justify-between items-end mb-12">
			<div class="grow">
				<?php if ( $ub_title ) : ?>
					<h2 class="mb-2 text-2xl font-bold tracking-tight leading-none lg:text-3xl" style="max-width: <?php echo esc_attr( $ub_title_rem ); ?>;"><?php echo esc_html( $ub_title ); ?></h2>
				<?php endif; ?>
				...
				<?php if ( $ub_link ) : ?>
					<div class="inline-block mt-6">
						<a href="<?php echo esc_url( $ub_link['url'] ); ?>" 
							...
							class="inline-flex gap-4 items-center px-4 py-2 text-xs font-bold text-white shadow-lg transition-all duration-300 rounded-xs bg-primary shadow-primary/20 hover:bg-primary-dark">
			...
			<div class="flex gap-2 items-center pb-2 shrink">
				<button data-testimonials-carousel-prev class="flex justify-center items-center w-12 h-12 bg-white rounded-full border border-white shadow-sm transition-all cursor-pointer group shadow-primary/5 hover:border-primary hover:bg-primary">
				...
				<button data-testimonials-carousel-next class="flex justify-center items-center w-12 h-12 bg-white rounded-full border border-white shadow-sm transition-all cursor-pointer group shadow-primary/5 hover:border-primary hover:bg-primary">
			...
			<div class="swiper-wrapper flex! gap-[10px] group-[.swiper-initialized]:gap-0">
				<?php foreach ( $ub_testimonials as $item ) : ?>
					<div class="swiper-slide flex h-auto! w-full max-w-[300px] flex-col">
						<div class="flex overflow-hidden flex-col gap-4 justify-between p-6 h-full bg-white rounded-sm border transition-all duration-300 group/item grow border-slate-50 hover:shadow-md">
							<!-- Quote. -->
